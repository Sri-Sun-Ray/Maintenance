...
try {
    ...
    foreach ($modules as $module) {
        $stmt = $conn->prepare("SELECT s_no, module, details, name_number, date_commission, required_value, observed_value, remarks, image_path 
                            FROM {$module}
                            WHERE station_info_id = ? ORDER BY CAST(s_no AS SIGNED)");
        if (!$stmt) {
            continue;
        }
