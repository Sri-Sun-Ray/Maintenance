...
try {
    ...
    // Consistent escaping for lookup
    ...
    foreach ($modules as $module) {
        if ($module === 'quarterly_check') {
            $sql = "SELECT s_no, module, details, name_number, date_commission, required_value, observed_value, remarks, image_path 
                    FROM quarterly_check WHERE station_info_id = ? ORDER BY CAST(s_no AS SIGNED)";
        } else {
            $sql = "SELECT s_no, ? AS module, maintenance_task_description AS details, location AS name_number, frequency AS date_commission, action_taken AS required_value, equipment_condition AS observed_value, remarks, image_path 
        ...
        // Ensure we retrieve all possible rows (increased limits)
        $maxRows = ($module === 'quarterly_check') ? 25 : 50; 
        
        for ($sNo = 1; $sNo <= $maxRows; $sNo++) {
            if (isset($savedRows[$sNo])) {
                $row = $savedRows[$sNo];
                if (isset($defaultValues[$module][$sNo])) {
                    if (empty($row['details'])) $row['details'] = $defaultValues[$module][$sNo]['details'];
                    if (empty($row['required_value'])) $row['required_value'] = $defaultValues[$module][$sNo]['required_value'];
                }
