  ...
  <div style="overflow-x: auto;">
    <table class="obs-table">
      ...
      <tbody>`;
                    ...
                    rows.forEach(obs => {
                        const imagePaths = Array.isArray(obs.image_paths)
                            ? obs.image_paths.filter(p => !!p)
                            : (obs.image_path ? [obs.image_path] : []);
                        const imgHtml = imagePaths.length
                            ? imagePaths.map(path => `<img src="${escapeHtml(path)}" class="img-thumb" alt="img" onclick="openImageModal('${escapeHtml(path)}')">`).join('')
                            : 'N/A';
                        html += `<tr>
